...
def model_fn_decorator(criterion):
    ModelReturn = namedtuple("ModelReturn", ["preds", "loss", "acc", "confusion_matrix"])
    ...
    def model_fn(model, data, epoch=0, eval=False):
        with torch.set_grad_enabled(not eval):
            ...
            cm_ = confusion_matrix(labels_np.ravel(), preds_np.ravel(), labels=list(range(9)))

        return ModelReturn(preds, loss, {"acc": acc.item(), "loss": loss.item()}, cm_)
